...
**단, 판정을 통과한 후보가 있는데 빈 목록이면 위반이다**(2026-09-19 추가). `조건 판정` 단계가
남기는 `통과 N개`로 본다. 빈 추천은 가격·제외·형식 검사를 거의 자동으로 통과하므로, 이 검사가
없으면 아무것도 추천하지 않는 쪽이 통과율에서 유리해진다.
...
1. **constraints** — 필수 조건 준수. 코드로만 본다.
   ...
   - 공통: 판정을 통과한 후보가 있으면 빈 목록이 아님. `조건 판정` 단계의 detail(`통과 25개 중
     2개 추천, 제외 5개`)에서 통과 후보 수를 읽는다. 원본 저장소의 문구(`통과 25개 중 2개 선택`)도
     같은 식으로 읽히고, 이 수를 남기지 않은 옛 기록은 검사하지 않는다. 에이전트가 되물음 뒤에도
     비운 경우까지 위반으로 세므로, 정당한 빈 추천인지는 기록의 `answer`를 읽고 판단한다.
...
`summary.json`은 전체 수치와 함께 **추천을 낸 문항만의 수치**(`with_recommendations`: 문항 수·
통과 수·지연 중앙값·심판 평균)와 **통과 후보가 있었던 빈 추천 수**(`overlooked_empty`)를 낸다.
빈 추천은 세 축을 거의 자동으로 통과하고, 심판 점수가 만점이며, 빨리 끝난다. 빈 추천이 많을수록
...
- **통과율은 추천 0개 건수와 함께 읽는다.** 실행끼리 비교할 때는 전체 통과 수보다
  `with_recommendations`(추천을 낸 문항)와 추천을 낸 문항 수를 먼저 본다.
